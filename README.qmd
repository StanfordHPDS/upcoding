...
-   **Simulate baseline co-occurring health conditions** for a user-defined number of individuals
    -   Users have three options to use as a reference for simulating co-occurring HCCs: (1) the default, `coocurring_v28_hcc_counts_allofus`, (2) `cms_hcc_v28_reference_hcc_count_dt`, or (3) a custom reference of co-occurring HCCs in the same format as these two files. To read more about the details of each of the built-in references, users can either type `help()` for each reference (e.g. `help(coocurring_v28_hcc_counts_allofus)` ) after loading the package or look at the underlying documentation in `R/data.R`. 
